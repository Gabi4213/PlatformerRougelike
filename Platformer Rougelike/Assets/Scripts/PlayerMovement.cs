...
public class PlayerMovement : MonoBehaviour
{
    ...
    {
		return Physics2D.OverlapBox(_groundCheckPoint.position, _groundCheckSize, 0, _groundLayer) && !IsJumping;
	...
	private void Update()
	{
        ...
        {
			Instantiate(runFX, transform.position - (Vector3.up * transform.localScale.y / 2.2f) + (new Vector3(0.8f, 0, 0)), Quaternion.identity);
		}
		if (Input.GetKeyDown(KeyCode.A) && IsGrounded())
		{
			Instantiate(runFX, transform.position - (Vector3.up * transform.localScale.y / 2.2f) + (new Vector3(-0.8f,0,0)), Quaternion.Euler(new Vector3(0,-180,0)));
		}
